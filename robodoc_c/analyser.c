 ...
 *
 *****
 * $Id: analyser.c,v 1.75 2008/03/26 15:56:13 thuffir Exp $
 */
 ...
 */
{
    ...
    // AMVED MODIFICATION
    // Added next 2 lines (MILIND GUPTA 5/8/2010)
    cur_char = header_name;
	for ( ; *cur_char !='\0'; ++cur_char);

	for ( ; *cur_char != '/' && *cur_char !='.' ; --cur_char);		// Added this and commented next (AMVED MODIFICATION)
 ...
 */
{
    ...
    // AMVED MODIFICATION
    // Added for reverse searching (MILIND GUPTA 5/8/2010)
    int				   namelen;
    int				   count;
    //////////////////////////////////////////////////////
    ...
    if ( ( cur_char = header_name ) != NULL )
    {
    	...
    	cur_char = header_name;
    	/////////////////////////////////////////
    	for (count=namelen; count>0; count--)          // Added this and commented next (AMVED MODIFICATION)
        //for ( ; *cur_char != '\0'; ++cur_char )
        {
    		if ('/' == cur_char[count] || '.'== cur_char[count])			// Added this and commented next (AMVED MODIFICATION)
            //if ( '/' == *cur_char )
            {
                if (cur_char[count+1])			// Added this and commented next 2 (AMVED MODIFICATION)
    			//++cur_char;
                //if ( *cur_char )
                {
                    name = cur_char + count + 1;
                    break;
                }
            }
        }
