    ...
    char *srcRoot)
{
    ...
    fprintf( dest_doc, "</title>\n" );
    // AMVED MODIFICATION
    // MILIND GUPTA 6/18/2010
    // Added the next 8 lines of code to add all titles to the section
    for (count=1;count<cur_header->no_names;count++)
    {
        fprintf( dest_doc, "%s", "<title>" );
        RB_XMLDB_Generate_String( dest_doc, cur_header->module_name);
        fprintf( dest_doc, "%s", "/" );
        RB_XMLDB_Generate_String( dest_doc, cur_header->names[count]);
        fprintf( dest_doc, "%s", "</title>\n" );
    }
    // AMVED MODIFICATION
    // MILIND GUPTA 5/8/2010
    // Added the next 10 lines of code to add letter and type of element
    // TO tell which file is the header is under
    fprintf( dest_doc, "<fileName path=\"");
    RB_XMLDB_Generate_String( dest_doc, replace_str(cur_header->owner->filename->path->name,srcRoot,""));
    fprintf( dest_doc, "\">");
    RB_XMLDB_Generate_String( dest_doc, cur_header->owner->filename->name);
    fprintf( dest_doc, "</fileName>\n");

    fprintf( dest_doc, "<type letter=\"");
    RB_XMLDB_Generate_String( dest_doc, (char*)&(cur_header->htype->typeCharacter));
    fprintf( dest_doc, "\">");
    RB_XMLDB_Generate_String( dest_doc, cur_header->htype->indexName);
    fprintf( dest_doc, "</type>\n");
}
...
void RB_XMLDB_Generate_BeginSection(
    ...
    char *name,
    struct RB_header *cur_header,
    char *srcRoot) // ADDED srcRoot (MILIND GUPTA 5/9/2010 AmVed)
{
	short count;
    USE( depth );
